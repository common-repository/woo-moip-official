		...
		if ( ! $setting->is_checkout_default() ) {
			if ( ! is_null( $payment_link ) ) {
				printf(
					...
					__( 'Your order has been processed successfully, click the link below to make the payment.', 'woo-moip-official' ),
					$payment_link,
					$model->is_bankslip_payment() ? __( 'Print', 'woo-moip-official' ) : __( 'Pay now', 'woo-moip-official' )
				);
			}
		} else {
