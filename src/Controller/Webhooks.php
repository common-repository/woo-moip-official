...
class Webhooks
{
	...
	public function handle()
	{
		...
		try {
			...
			if ( $this->setting->is_enabled_logs() ) {
				$this->setting->log()->add( 'moip-brazil-official-webhook', 'WEBHOOK BODY: ' . json_encode( $body ) );
			}

		} catch ( Exception $e ) {
			if ( $this->setting->is_enabled_logs() ) {
				$this->setting->log()->add( 'moip-brazil-official-webhook-error', 'WEBHOOK HANDLE ERROR: ' . $e->__toString() );
			}
	...
	private function _proccess_event( $body )
	{
		if ( ! isset( $body->event ) || empty( $body->event ) || ! $this->_is_valid_token() ) {
			if ( $this->setting->is_enabled_logs() ) {
				$this->setting->log()->add( 'moip-brazil-official-webhook-error', 'INVALID WEBHOOK' );
			}
			throw new Exception( __( 'Invalid webhook.', 'woo-moip-official' ) );
		}
	...
	public static function set_token( $custom_gateway )
	{
		...
		if ( ! $moip_sdk->is_valid() ) {
			$custom_gateway->add_error( __( 'Authentication error.', 'woo-moip-official' ) );
		...
		try {
			...
			$custom_gateway->add_error( __( 'Enable payment in the field below.', 'woo-moip-official' ) );
	...
	public function admin_ipn_test()
	{
		...
		if ( ! current_user_can( 'manage_woocommerce'  ) ) {
			wp_send_json_error(
				array(
	               'text'  => __( 'You are not allowed to do this.', 'woo-moip-official' ),
	               'class' => 'alert-danger',
				)
			);
		}
		...
		if ( empty( $resource_id ) ) {
			wp_send_json_error(
				array(
	               'text'  => __( 'This resource id field is required.', 'woo-moip-official' ),
	               'class' => 'alert-warning',
				)
			);
		}
		...
		if ( empty( $notifications->webhooks ) ) {
			wp_send_json_success(
				array(
		           'text'  => __( 'This invalid resource id.', 'woo-moip-official' ),
		           'class' => 'alert-danger',
				)
			);
		}
		...
		$not_sent_args      = array(
           'text'  => __( 'No notifications to send.', 'woo-moip-official' ),
           'class' => 'alert-info',
		);
		...
		wp_send_json_success(
			array(
	           ...
	           'text'  => sprintf(
		           	...
		           	__( 'Webhook notification successfully sent.', 'woo-moip-official' ),
		           	__( 'Events', 'woo-moip-official' ),
